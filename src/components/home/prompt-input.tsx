...
export const PromptInput: React.FC<PromptInputProps> = ({
  ...
  const starterPrompts = [
    {
      ...
      prompt:
        "Build a modern glassmorphic SaaS analytics dashboard with dark theme, revenue charts, activity log, and metric cards.",
      icon: <BarChart3 className="h-3.5 w-3.5 text-orange-400" />,
    },
    {
      title: "Dark Auth Modal",
      prompt:
        "Create a sleek authentication modal with social logins, passwordless magic link input, and subtle orange border glow.",
      icon: <ShieldCheck className="h-3.5 w-3.5 text-red-400" />,
    },
    {
      title: "Interactive Pricing Table",
      prompt:
        "Design a 3-tier dark theme pricing section with monthly/yearly toggle, feature matrix, and glowing CTA buttons.",
      icon: <Flame className="h-3.5 w-3.5 text-amber-400" />,
    },
    {
      title: "Developer Portfolio Hero",
      prompt:
        "Generate a minimalist developer portfolio hero section with interactive code preview, tech stack icons, and floating dock.",
  ...
  return (
    <div
      className={cn(
        "w-full max-w-4xl mx-auto flex flex-col gap-4 z-20",
        className,
      )}
    >
          ...
          <div className="flex flex-wrap items-center justify-between gap-3 pt-3 border-t border-zinc-800/70">
            ...
            <div className="flex items-center gap-3">
              <span className="hidden sm:inline-block text-[11px] text-zinc-500 font-mono">
                Press{" "}
                <kbd className="px-1 py-0.5 rounded bg-zinc-800 text-zinc-400 border border-zinc-700">
                  ⌘
                </kbd>{" "}
                +{" "}
                <kbd className="px-1 py-0.5 rounded bg-zinc-800 text-zinc-400 border border-zinc-700">
